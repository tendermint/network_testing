...
rm $NODE_DIR/priv_validator.json
...
docker cp $NODE_DIR/core/ proxy_tmcommon:/data/tendermint/core

# fix up perms
docker run --rm --volumes-from proxy_tmcommon -u root tendermint/tmbase chown -R tmuser:tmuser /data/tendermint

# start the proxy
docker run -d -p 46657:46657 --name proxy_tmnode --volumes-from proxy_tmcommon -e TMNAME=$NAME -e TMSEEDS=$TMSEEDS -e TMROOT=/data/tendermint/core -e PROXYAPP=nilapp tendermint/tmbase /data/tendermint/core/init.sh
