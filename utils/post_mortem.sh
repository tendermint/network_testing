...
mintnet docker --machines ${MACH_PREFIX}[1-${N}] -- cp bench_app_tmcore:/data/tendermint/core/data/cswal cswal
...
for i in `seq 1 $N`; do
	...
	docker-machine ssh $mach docker logs bench_app_tmcore &> $RESULTS/$i/tendermint.log
